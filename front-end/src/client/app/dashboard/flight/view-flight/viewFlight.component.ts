...
export class ViewFlightComponent {
  ...
  private passengersOnFlight : any;
  ...
  updateFlightToView(value : string) {
    this._apiService.post('http://localhost:8080/user/listFlights', {}).subscribe(response => {
      ...
      (<HTMLInputElement>document.getElementById('ticketprice-input')).value = temp.data[index].ticketPrice;
      this.passengersOnFlight = temp.data[index].passengers;
    });
  }
